...
class SaleFormat extends Command
{
	 ...
	 * @var string
	 */
	protected $signature='PZJota:saleFormat';
		...
		foreach ($dates AS $date) {
			...
			$productByInvoices=ProductsByInvoice::whereHas('invoice',function ($i) use ($sysconf,$datei,$datef) {
				$i->whereBetween('date',[$datei->firstOfMonth()->toDateString(),$datef->endOfMonth()->toDateString()])
                    ...
                    ->where('ind_estado','aceptado')->whereHas('sale',function ($r){
			...
			$local=Storage::disk('local')->path("FIFCO".DIRECTORY_SEPARATOR."PZJota".DIRECTORY_SEPARATOR."salesFormat.txt");
